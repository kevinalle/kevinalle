...
class Lapack(Frame):
    ...
    def check_lapack(self):
      ...
      stop
        ...
        fcomm = self.mpif77+' '+Frame.fcflags+' -o tmpf '+'tmpf.f '+self.lapacklib+' '+self.blaslib+' '+self.ldflags_f77
    ...
    def down_install_lapack(self):
        ...
        # Old way to go: os.chdir(os.path.join(os.getcwd(),'lapack-3.2.1'))
        comm = 'ls -1 | grep lapack- | grep -v sca'
        (output, error, retz) = runShellCommand(comm)
        if(retz != 0):
		    print '\n\nLAPACK: error changing to LAPACK dir'
		    print 'stderr:\n','*'*40,'\n','   ->  no lapack directory found','\n','*'*40
		    sys.exit()
        rep_name = output.replace ("\n","")
        print 'Installing ',rep_name,'...'
        os.chdir(os.path.join(os.getcwd(),rep_name))

        # compile and generate library
        self.set_etime()
        self.write_makeinc()

        print 'Compiling ',rep_name,' (this will take several minutes)...',
        sys.stdout.flush()
        comm = self.make+' lib'
        (output, error, retz) = runShellCommand(comm)
        if(retz != 0):
    ...
    def set_etime(self):
      ...
      T1 = ETIME_( TARRAY )
      END\n""")
        fcomm = self.f90+' '+self.f90flags+' -o tmpf tmpf.f'
        (output, error, retz) = runShellCommand(fcomm)
