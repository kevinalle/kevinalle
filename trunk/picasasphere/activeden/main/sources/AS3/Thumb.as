package  {
	...
	public class Thumb extends flash.display.Sprite {
		...
		protected function over(evt : flash.events.MouseEvent) : void {
			...
			this.removeEventListener(flash.events.Event.ENTER_FRAME,this.shrink);
			this.addEventListener(flash.events.Event.ENTER_FRAME,this.grow);
		...
		protected function out(evt : flash.events.MouseEvent) : void {
			...
			this.removeEventListener(flash.events.Event.ENTER_FRAME,this.grow);
			this.addEventListener(flash.events.Event.ENTER_FRAME,this.shrink);
		}
		
		protected function grow(e : flash.events.Event) : void {
			if(this.pos.norm() < this.r * 1.2) this.pos.scaleBy(1.04);
		}
		
		protected function shrink(e : flash.events.Event) : void {
			if(this.pos.norm() > this.r) this.pos.scaleBy(.96);
			else this.removeEventListener(flash.events.Event.ENTER_FRAME,this.shrink);
		}
